...
@pytest.fixture()
def service(session_factory: sessionmaker[Session]) -> SightseeingsService:
    return SightseeingsService(session_factory)
...
def test_get_sightseeings_pagination(service: SightseeingsService, session_factory: sessionmaker[Session]) -> None:
...
def test_get_sightseeing_by_id_found_and_not_found(service: SightseeingsService, session_factory: sessionmaker[Session]) -> None:
    items = seed(session_factory)
...
def test_add_sightseeing_persists_and_returns_id(service: SightseeingsService, session_factory: sessionmaker[Session]) -> None:
...
def test_try_update_sightseeing_returns_true_when_updated(service: SightseeingsService, session_factory: sessionmaker[Session]) -> None:
    items = seed(session_factory)
...
def test_try_update_sightseeing_returns_false_when_missing(service: SightseeingsService) -> None:
    ok = service.try_update_sightseeing(12345, Sightseeing("NA", "NA"))
    assert ok is False


def test_try_delete_sightseeing_returns_true_and_removes(service: SightseeingsService, session_factory: sessionmaker[Session]) -> None:
    items = seed(session_factory)
...
def test_try_delete_sightseeing_returns_false_when_missing(service: SightseeingsService) -> None:
